...
- `servicios/pantalla/` — cola de pantallas compartida por QuickSettings y Ajustes > Pantalla. `modes.ts` es la lógica pura (testeada con node); `service.ts` tiene el poller, `applyPatch` (aplica en vivo con `hyprctl eval 'hl.monitor{…}'` — bajo config Lua `hyprctl keyword` ya no existe) y la persistencia. **Persiste por partida doble**: `~/.config/gigios/display.json` (fuente de verdad de AGS) **y** `~/.config/hypr/monitor-settings.lua`, un chunk Lua generado que carga `hyprland.lua`. Lo segundo no es redundante: Hyprland no lee el JSON, así que sin ese fichero un **`hyprctl reload`** releía el comodín de `gigios/monitores.lua` — `preferred`/escala 1 — y tiraba la pantalla de 240 Hz a 60 y de escala 1.25 a 1, sin que AGS pudiera enterarse (no hay señal de recarga; el poller solo observa). Se carga con `util.carga_opcional` (`dofile` + `pcall`), así que su ausencia **ya no es error duro** como lo era un `source =` — pero sí devolvería la pantalla al comodín, de ahí que se versione aunque se regenere solo. Detalle en el `CLAUDE.md` raíz.
