...
- `servicios/energia/botonEncendido.ts` — **qué hace el botón de encendido físico** (Ajustes > Energía). El shell **solo persiste la elección** (`botonApagado` en `preferences.json`); quien la ejecuta es `hypr/scripts/boton-apagado.sh` desde un `bindl = , XF86PowerOff` de Hyprland, releyéndola en cada pulsación — así el botón responde con AGS caído y con la sesión bloqueada, y el setter no tiene que relanzar ni recargar nada. La única acción que vuelve al shell es `menu`, por el `request` **`toggle-power-menu`** (`alternarMenuEnergia()`), añadido con ella.
...
- `modulos/ajustes/preferences.ts` — preferencias globales del shell que persisten en `~/.config/gigios/preferences.json`, a diferencia del estado solo en RAM de `modulos/barra/funciones/estado.ts`. Incluye los toggles `startupVolumeMuted` y `startupMicMuted`: `inicializador/init.sh` los lee al arrancar, espera a que WirePlumber publique cada endpoint predeterminado y fuerza su mute según el valor. Para añadir una preferencia: crea un `createState`, léela en `load()`, escríbela en `save()` y expón un setter que llame a `save()`.
  **`absorberSuperSinAtajo`** es de las que se aplican en caliente: su setter llama a
  `hypr/scripts/generar-nop-binds.sh on|off`, que regenera `hypr/keybinds-nop.conf` (los binds que
  absorben SUPER + tecla sin atajo, para que no se escriba en la aplicación) y recarga Hyprland él
  mismo. **Activarlo REGENERA**, así que es también el gesto con el que recoger atajos nuevos de
  `keybinds.conf`; al desactivarlo el fichero se queda con comentarios y no se borra — un `source =`
  a un fichero ausente es error duro en Hyprland. Detalle completo en el `CLAUDE.md` raíz.
