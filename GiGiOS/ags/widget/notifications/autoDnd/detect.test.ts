...
test("a fullscreen client whose class is listed matches", () => {
  assert.equal(matchesFullscreenApp({ class: "mpv", fullscreen: 1 }, ["mpv"]), true)
})

test("matching is a case-insensitive substring", () => {
  assert.equal(matchesFullscreenApp({ class: "org.Mozilla.Firefox", fullscreen: 1 }, ["firefox"]), true)
  assert.equal(matchesFullscreenApp({ class: "MPV", fullscreen: 1 }, ["mpv"]), true)
})

test("initialClass / initial_class are also considered", () => {
  assert.equal(matchesFullscreenApp({ initialClass: "vlc", fullscreen: 1 }, ["vlc"]), true)
  assert.equal(matchesFullscreenApp({ initial_class: "vlc", fullscreen: 1 }, ["vlc"]), true)
})

test("a fullscreen client whose class is NOT listed does not match", () => {
  assert.equal(matchesFullscreenApp({ class: "kitty", fullscreen: 1 }, ["mpv", "vlc"]), false)
})

test("empty / whitespace app entries never match (no accidental universal match)", () => {
  assert.equal(matchesFullscreenApp({ class: "kitty", fullscreen: 1 }, [""]), false)
  assert.equal(matchesFullscreenApp({ class: "kitty", fullscreen: 1 }, ["  "]), false)
})

test("a fullscreen client with no class does not match", () => {
  assert.equal(matchesFullscreenApp({ fullscreen: 1 }, ["mpv"]), false)
})
...
test("a configured fullscreen app silences even if it is a game-blocklisted app", () => {
  ...
  assert.equal(shouldSilence([{ class: "mpv", fullscreen: 1 }], ["mpv"]), true)
})

test("a fullscreen app that is NOT configured does not silence on its own", () => {
  assert.equal(shouldSilence([{ class: "mpv", fullscreen: 1 }], []), false)
})

test("silences when any one client in the list qualifies", () => {
  assert.equal(shouldSilence([
    { class: "kitty" },
    { class: "firefox", fullscreen: 1 },
    { class: "mpv", fullscreen: 1 },
  ], ["mpv"]), true)
})
...
test("a configured fullscreen app off the focused workspace does not silence", () => {
  assert.equal(shouldSilence(
    [{ class: "mpv", fullscreen: 1, workspace: { id: 3 } }],
    ["mpv"],
    2,
  ), false)
})
...
test("silences only when the qualifying client is on the focused workspace", () => {
  const clients = [
    ...
    { class: "firefox", fullscreen: 1, workspace: { id: 2 } }, // browser on ws 2
  ]
