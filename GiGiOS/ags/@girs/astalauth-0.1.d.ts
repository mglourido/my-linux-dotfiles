...
export namespace AstalAuth {
    ...
    /**
     * For simple authentication using only a password, using the {@link AstalAuth.Pam.authenticate}
     ...
     * {@link AstalAuth.Pam.supply_secret} call. The secret is a string containing the user input. For
     * [auth-info]`AstalAuth.Pam::auth-info:` and [auth-error]`AstalAuth.Pam::auth-error:`
     * it should be `NULL`. Not connecting those signals, is equivalent to calling
     * {@link AstalAuth.Pam.supply_secret} with `NULL` immediately after the signal is emitted.
     * 4. start authentication process using {@link AstalAuth.Pam.start_authenticate}.
     * 5. it is possible to reuse the same Pam object for multiple sequential authentication attempts.
     * Just call {@link AstalAuth.Pam.start_authenticate} again after the `success` or `fail` signal
     * was emitted.
     * @gir-type Class
     */
    class Pam extends GObject.Object {
        ...
        // Signals
        /** @signal */
        connect<K extends keyof Pam.SignalSignatures>(signal: K, callback: GObject.SignalCallback<this, Pam.SignalSignatures[K]>): number;
        connect(signal: string, callback: (...args: any[]) => any): number;

        /** @signal */
        connect_after<K extends keyof Pam.SignalSignatures>(signal: K, callback: GObject.SignalCallback<this, Pam.SignalSignatures[K]>): number;
        connect_after(signal: string, callback: (...args: any[]) => any): number;

        /** @signal */
        emit<K extends keyof Pam.SignalSignatures>(signal: K, ...args: GObject.GjsParameters<Pam.SignalSignatures[K]> extends [any, ...infer Q] ? Q : never): void;
