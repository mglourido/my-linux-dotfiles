...
function spin(min: number, max: number, valor: number, ancho: number): Gtk.SpinButton {
...
export function crearCampoFecha(inicial: string, alCambiar: (iso: string) => void): CampoFecha {
  const partes = desdeFechaISO(inicial) ?? { anio: 2026, mes: 1, dia: 1 }

  const dia = spin(1, 31, partes.dia, 2)
  const mes = spin(1, 12, partes.mes, 2)
  const anio = spin(1970, 2100, partes.anio, 4)
  ...
  const widget = (
    <box cssClasses={["cal-campo-fecha"]} spacing={3}>
      {dia}
...
export function crearCampoHora(inicial: string, alCambiar: (hora: string) => void): CampoHora {
  const minutosIniciales = Math.max(0, horaAMinutos(inicial))
  const horas = spin(0, 23, Math.floor(minutosIniciales / 60), 2)
  const minutos = spin(0, 59, minutosIniciales % 60, 2)
  ...
  const widget = (
    <box cssClasses={["cal-campo-hora"]} spacing={3}>
      {horas}
      <label cssClasses={["cal-campo-sep"]} label=":" />
      {minutos}
    </box>
