...
import BotonItemTray from "./BotonItem"
...
export default function OverflowTray({
  ...
  const salir = () => {
    dentroOverflow = false
    autoCierre.onLeave()
  }
  const alCambiarMenuHijo = (abierto: boolean) => {
    if (eliminado) return
    hijosAbiertos = Math.max(0, hijosAbiertos + (abierto ? 1 : -1))
    if (!abierto && hijosAbiertos === 0 && !dentroOverflow) autoCierre.onLeave()
  }

  const filas = items((lista) => agruparEnFilas(lista, 4))
  const rejilla = (
